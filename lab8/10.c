...
int findHighestSalaryEmployee()
{
    int i, c = 0;

    for (i = 1; i < NUM_EMPLOYEES && employees[i].name; i++)
        if (employees[i].salary > employees[c].salary)
            c = i;
    return c;
}

int findYoungestEmployee()
{
    int i, c = 0;

    for (i = 1; i < NUM_EMPLOYEES && employees[i].name; i++)
        if (employees[i].birthyear > employees[c].birthyear)
            c = i;
    return c;
}

void deleteEmployee(const int i)
{
    int j;
    for (j = i + 1; j < NUM_EMPLOYEES && employees[j].name; j++)
        ;
    swap(employees + i, employees + j - 1);
